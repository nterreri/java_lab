...
public class chessboard {
	...
	*/
	...
	/***
	 * Initializes chessboard with <code>ROWS</code>-many rows and <code>COLUMNS</code>-many columns,
	 * placing black pieces at the top of the board and white pieces at the
	 * bottom.
	 * 
	 * @param chessboard a two-dimensional array representing the chessboard
	 */
	 ...
	 * <p>
	 * Finally, if the move is legitimate, it executes it, then increases the turn counter.
	 * <p>
	 * @param chessboard	a two-dimensional array representing the chessboard
	 * @param move			a string expressing a move, formatted as, e.g., a1 to a5 or a1 to c3
	 */
	public static void move(Chessmen[][] chessboard, String move)
	{
		...
		{
			System.err.println("Invalid final position.");
		}
		...
		{
			System.err.println("Selected initial position is empty.");
		}
		else if(isWhiteTurn())
		{
			if(!isWhite(chessboard[initial_x][initial_y]))
			{
				System.err.println("Piece selected is black on a white turn.");
				System.out.println("Please, select a white piece.");
			}
			else
			{
				if(!isMoveLegal(chessboard, 
						initial_x, initial_y, final_x, final_y))
					System.err.println("Illegal move: patter invalid"
							+ " or path is not free.");
				else
				{
					chessboard[final_x][final_y] = chessboard[initial_x][initial_y];
		...
		else //if(!isWhiteTurn())
		{
			if(isWhite(chessboard[initial_x][initial_y]))
			{
				System.err.println("Piece selected is white on a black turn.");
				System.err.println("Please, select a black piece.");
			}
			else
			{
				if(!isMoveLegal(chessboard, 
 ...
 * Returns true if the move specified in terms of initial coordinates
			...
			int ix, int iy, int fx, int fy)
	{
		...
		case BLACK_PAWN:
			...
			if(patternIsLegal(chessboard[ix][iy], ix, iy, fx, fy))
			{
				...
				{
					System.err.println("Black pawn cannot move forward on top"
							+ " of white piece");
					return false;
				}
				else
		...
		case WHITE_PAWN:
			...
			if(patternIsLegal(chessboard[ix][iy], ix, iy, fx, fy))
			{
				...
				{
					System.err.println("White pawn cannot move forward on top"
							+ " of black piece");
					return false;
				}
				else
	...
	public static int find_xcoord(String position_on_chessboard)
	{
		...
		case 'h':
			return 7;
		default:
			System.err.println("Invalid chessboard position:");
			return -1;
		}
	}
 ...
 */
		...
		if(!white_alive)
			{
				...
				System.out.println("Black side victory.");
				return -1;
			}
		else if(!black_alive)
		{
			System.out.println("Game over.");
			System.out.println("White side victory.");
			return -2;
		}
		else return 0;
	}
	...
	public static void main(String[] args) {
		...
		while(true)
		{
			...
			System.out.println("Enter a command (e.g. \"e1 to e5\"):");
			command = is.nextLine();
